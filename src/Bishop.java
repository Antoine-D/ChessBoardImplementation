...
public class Bishop extends ChessPiece 
{
			...
			BoardState stateOfBoard) 
	{
		...
		// Check for each of the four diagonal directions that the bishop can move in
		for(int columnIncrement = -1; columnIncrement <= 1; columnIncrement+=2)
		{
			for(int rowIncrement = -1; rowIncrement <= 1; rowIncrement+=2)
			{
				// Set the diagonal iterator to one past the bishop's current position
				int columnIterator = super.pieceLocation.get("Column") + columnIncrement;
				int rowIterator = super.pieceLocation.get("Row") + rowIncrement;
				
				while(rowIterator < 8 && columnIterator < 8)
				{
					ChessPiece pieceAtPosition = stateOfBoard.getPieceAtPosition(rowIterator, columnIterator);
					
					// If a piece exists at the position, then check if it's one of your own 
					// and stop checking this diagonal.
					if(pieceAtPosition != null)
					{
						// If the piece is not one of your own, then can take it (so add to possible move)
					...
					// If no piece at the position then add the position to the list and continue iterating down the diagonol
					else
					{
						Map<String, Integer> possibleMoveToPosition = new HashMap<String, Integer>();
						possibleMoveToPosition.put("Column", columnIterator);
